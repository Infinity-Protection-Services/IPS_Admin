...
function NewJobsTable(props) {
  ...
  useEffect(() => {
    ...
    if (props.GetAllJobPostAdmin) {
      setData({
        ...data,
        rows: props.GetAllJobPostAdmin.data.new_job_data.map((item, index) => {
          const Date1 = commonMethods.getSortedDate(item.job_schedule_time)
          item['index'] = index + 1
          item["full_name"] = commonMethods.ConvertToCamelCase(item.requester_first_name + " " + item.requester_last_name)
          ...
          // item["date"] = Date1.sort(function (x, y) { return (new Date(y["start_date"]).getTime()) - (new Date(x["end_date"]).getTime()) });
          item['amount'] = "$" + item.recommended_rate
  ...
  useEffect(() => {
    const columns = [
      ...
      { label: props.t('Requester'), field: 'full_name', },
      { label: props.t('Company Name'), field: 'comp_name' },
      { label: props.t('Recommended Rate'), field: 'amount', sort: 'disabled' },
      ...
      {(!props.jobDetailsVisibility && !props.EditJob) &&
        <>
          {/* <div>
                    ...
                    {props.EditJob}
                    {(!props.jobDetailsVisibility || !props.EditJob) ? "true" : "false"}
                </div> */}
          ...
          <div className="customer-support-table-wrapper-new common-wrapper p-r new-jobs-table-wrapper order-table">
            <MDBDataTable
              ...
              noRecordsFoundLabel={<div className="no-data-found ta-l">{props.t("No new jobs found!")}</div>}
...
export default withNamespaces()(connect(mapStateToProps, mapDispatchToProps)(NewJobsTable))
