...
function IncompleteJobDetails(props) {
  const { jobdetails, PaymentMode, PaymentColorCode, StatusColorCode, Status } =
    props;
  let totalPayableAmount = jobdetails.recommended_rate;
  const [data, setData] = useState({
    PaymentHistory: { columns: [], rows: [] },
  });
  const [proData, setProData] = useState({
    ProviderDetails: { columns: [], rows: [] },
  });
  const [clockData, setClockData] = useState({
    ClockDetails: { columns: [], rows: [] },
  });
  const [timeSheetData, setTimeSheetData] = useState({
    Providers: { columns: [], rows: [] },
  });
  ...
  let refundData = {
    ...
    destinationId_backup: jobdetails.job_requests
      .filter(
        (x) =>
          x.is_backup_provider === 0 && x.provider_id === Number(provider_data)
      )
      .map((x) => x.stripeConnectedAccountId)
      .toString(),
    destinationId_provider: jobdetails.job_requests
      .filter(
        (x) =>
          x.is_backup_provider === 1 && x.provider_id === Number(provider_data)
      )
      .map((x) => x.stripeConnectedAccountId)
      .toString(),
    ...
    provider_name: jobdetails.job_requests.map(
      (x) => x.is_backup_provider === 0 && x.first_name + " " + x.last_name
    ),
    backup_provider_name: jobdetails.job_requests.map(
      (x) => x.is_backup_provider === 0 && x.first_name + " " + x.last_name
    ),
  };

  const location = useLocation();
  const Primary_provider = jobdetails.job_requests
    .filter(
      (x) => x.is_backup_provider === 0 && (x.status === 4 || x.status === 1)
    )
    .map((x) => x.provider_id);
  const Backup_provider = jobdetails.job_requests.filter(
    (x) => x.is_backup_provider === 1 && (x.status === 4 || x.status === 1)
  );
  const provider =
  ...
  let payableAmount = jobdetails.job_payments_data.map((item) => {
    return (item.topay =
      item.is_payment_by_admin === 1 &&
      item.status.toLocaleLowerCase() === "success"
        ? { paid: true, amount: item.total_amount }
        : { paid: false, amount: 0 });
  });
  ...
  jobdetails.job_payments_data.forEach((item) => {
    if (
      item.status.toLocaleLowerCase() === "success" &&
      item.is_payment_by_admin === 1 &&
      item.event_name.toLocaleLowerCase() !== "refund"
    ) {
  ...
  payableAmount.forEach((item) => {
    return item.paid
      ? (totalPayableAmount -= item.amount)
      : (totalPayableAmount += item.amount);
  });

  totalPayableAmount = parseFloat(
    jobdetails.commision_rate
      ? totalPayableAmount -
          (jobdetails.recommended_rate * jobdetails.commision_rate) / 100
      : totalPayableAmount
  ).toFixed(2);
    ...
    if (parseFloat(dialog_amount) > 0) {
      if (parseFloat(dialog_amount) > parseFloat(paymentData.amount)) {
        ...
        if (
          ...
          providerType &&
          parseFloat(dialog_amount) > parseFloat(paymentData.amount)
        ) {
          is_payable = false;
  ...
  const PaymentorRefundHandler = () => {
    ...
    if (parseFloat(dialog_amount) > 0) {
      ...
      if (
        confirmPayment.task === "pay" &&
        parseFloat(dialog_amount) > parseFloat(totalPayableAmount)
      ) {
        is_payable = false;
        props.actions.setredux(
          props.t("You can not pay amount greater than payable amount"),
          "error_message",
          actionTypes.ERROR_MESSAGE
        );
        setConfirmPayment({ ...confirmPayment, amount: dialog_amount });
        ...
        if (jobdetails.amount_to_refund) {
          if (
            parseFloat(dialog_amount) > parseFloat(jobdetails.amount_to_refund)
          ) {
            is_payable = false;
            props.actions.setredux(
              props.t(
                "You can not refund amount greater than refundable amount"
              ),
              "error_message",
              actionTypes.ERROR_MESSAGE
            );
  ...
  useEffect(() => {
      ...
      PaymentHistory: {
        ...
        rows: jobdetails.job_payments_data.map((item, index) => {
          ...
          item["description"] = item.description
            ? commonMethods.ConvertToCamelCase(item.description)
            : "-";
          item["transaction_auth"] =
            item.is_payment_by_admin === 1
              ? item.event_name.toLocaleLowerCase() === "refund" &&
                provider_data
                ? props.t("Me to ") + item.first_name + " " + item.last_name
                : props.t("Me to ") + item.first_name + " " + item.last_name
              : commonMethods.ConvertToCamelCase(
                  jobdetails.requester_first_name +
                    " " +
                    jobdetails.requester_last_name
                ) + " to me";
          item["transaction_date"] = moment(parseInt(item.createdAt)).format(
            "MMM DD, YYYY"
          );
          item["amount_received"] =
            item.is_payment_by_admin === 0
          ...
          item["pay_status"] = (
            <span
              className={
                String(item.status).toLocaleLowerCase() === "success"
                  ? "color-green"
                  : "color-red"
              }
            >
              {commonMethods.ConvertToCamelCase(item.status)}
            </span>
          );
          item["action"] =
            item.is_payment_by_admin === 0
    ...
    setProData({
      ProviderDetails: {
        ...
        rows: jobdetails.job_requests.map((item, index) => {
          item["index"] = index + 1;
          item["pro_name"] = commonMethods.ConvertToCamelCase(
            item.first_name + " " + item.last_name
          );
          item["pay_amt"] =
            item.recommended_rate === "NaN"
              ? "$" + 0
              : "$" + item.recommended_rate || "-";
          item["current_status"] = (
          ...
          item["payment"] =
            (item.recommended_rate > 1 && (
              <button
                className="btn-pay btn btn-secondary"
                onClick={() => {
                  setConfirmPayment({
                    visibility: true,
                    task: "pay",
    ...
    setClockData({
      ClockDetails: {
        ...
        rows: jobdetails.job_requests
          .filter(
            (x) =>
              x?.is_backup_provider === 1 &&
              x?.clock_in_clock_out_data?.length > 0
          )
          .map((item, index) => {
            item["index1"] = index + 1;
            item["back_name"] =
              (item?.clock_in_clock_out_data.length > 0 &&
                item?.first_name + " " + item?.last_name) ||
              "-";
            item["in_time"] =
              item?.clock_in_clock_out_data.map((x) =>
                moment(x.clock_in_time, ["HH:mm:ss"]).format("hh:mm A")
              ) || "-";
            item["out_time"] =
              item?.clock_in_clock_out_data.map((x) =>
                moment(x.clock_out_time, ["HH:mm:ss"]).format("hh:mm A")
              ) || "-";
            item["total_time"] =
              item?.clock_in_clock_out_data.map((x) => x.worked_hours) || "-";
            item["in_date"] =
              item?.clock_in_clock_out_data.map((x) =>
                moment(x.clock_in_date).format("MMMM DD,YYYY")
              ) || "-";
            item["out_date"] =
              item?.clock_in_clock_out_data.map((x) =>
                moment(x.clock_out_date).format("MMMM DD,YYYY")
              ) || "-";
            item["location"] =
              item?.clock_in_clock_out_data.map((x) => x.current_location) ||
              "-";
            return item;
          }),
      },
  ...
  newData
    .filter(
      (x) => x.box_id && x.box_id.length > 0 && x.is_backup_provider === 0
    )
    .forEach((x, index) => {
  ...
  if (
    Object.keys(all_selected_box).map((x) => Number(x)).length <
    newMultiple.length
  ) {
    newMultiple
      .filter(
        (x) =>
          !Object.keys(all_selected_box)
            .map((x) => Number(x))
            .includes(x.id)
      )
      .forEach((x) => {
    ...
    setTimeSheetData({
      Providers: {
        ...
        rows: mergedData?.map((item, index) => {
          item["index"] = index + 1;
          item["full_name"] =
            (item?.box_id?.length > 0 &&
              item?.first_name + " " + item?.last_name) ||
            "-";
          item["start_date"] =
            moment(item?.boxInfo?.start_date, ["YYYY-MM-DD"]).format(
              "MMM DD, YYYY"
            ) || "-";
          item["end_date"] =
            moment(item?.boxInfo?.end_date, ["YYYY-MM-DD"]).format(
              "MMM DD, YYYY"
            ) || "-";
          item["start_time"] =
            moment(item?.boxInfo?.start_time, ["HH:mm"]).format("hh:mm A") ||
            "-";
          item["end_time"] =
            moment(item?.boxInfo?.end_time, ["HH:mm"]).format("hh:mm A") || "-";
          item["job_dates"] =
            `${item?.boxInfo?.job_dates
              .map((x) => moment(x, ["YYYY-MM-DD"]).format("MMM DD, YYYY"))
              .join(", ")}` || "-";
          return item;
        }),
      },
    ...
    setData({
      PaymentHistory: {
        ...
        columns: [
          ...
          {
            label: props.t("Invoice No"),
            field: "invoice_no",
            sort: "disabled",
          },
          {
            label: props.t("Transaction Id"),
      ...
      ProviderDetails: {
        ...
        columns: [
          ...
          ...(totalPayableAmount > 0
            ...
            : []),
          // { label: props.t("Information"), field: "info" },
          // ...(location.pathname === "/inprogress-jobs" && totalPayableAmount > 0
          //   ? [
          //       {
          //         label: props.t("Payment"),
          //         field: "payment",
    ...
    setClockData({
      ClockDetails: {
        ...
        columns: [
          ...
          {
            label: props.t("Backup Provider Name"),
            field: "back_name",
            sort: "disabled",
          },
          {
            label: props.t("Clock-in Time"),
            field: "in_time",
            sort: "disabled",
          },
          {
            label: props.t("Clock-out Time"),
            field: "out_time",
            sort: "disabled",
          },
          {
            label: props.t("Total Time"),
            field: "total_time",
            sort: "disabled",
          },
          {
            label: props.t("Clock-in Date"),
            field: "in_date",
            sort: "disabled",
          },
          {
            label: props.t("Clock-out Date"),
            field: "out_date",
            sort: "disabled",
          },
        ],
      ...
      <div className="input-group Heading-Wrapper-container support-discussion">
        <div className="form-outline Heading-Wrapper w-100p">
          ...
          {location.pathname !== "/dispute" && totalPayableAmount > 0 && (
            <button
              className="btn-save add_button_new"
              onClick={() => {
                setConfirmPayment({
                  visibility: true,
                  task: "refund",
                  amount: jobdetails.amount_to_refund,
                });
              }}
            >
              {props.t("Refund")}
      ...
      <div className="order-detail-table-wrapper remove-border new-job-detail-table-wrapper">
            ...
            <div className="job-overview-wrapper">
              <div className="mt-13">
                <RenderDetails
                  ...
                  value={
                    commonMethods.ConvertToCamelCase(jobdetails.description) ||
                    "-"
                  }
                />
                <RenderDetails
                ...
                <RenderDetails
                  ...
                  value={
                    `E - ${jobdetails.requester_email} / M - ${jobdetails.requester_mobile_no}` ||
                    "-"
                  }
                />
                <RenderDetails
                ...
                <RenderDetails
                  ...
                  value={`$${jobdetails.recommended_rate.toFixed(2)}` || "-"}
                />
                  ...
                  <RenderDetails
                    ...
                    value={
                      <div className="d-flex">
                        ...
                        {totalPayableAmount > 0 && <div className="p-r"></div>}
                      </div>
                    }
                  />
                )}
                <RenderDetails
                  ...
                  value={`$${paid_amount.toFixed(2)}` || "-"}
                />
                {props.fromrefundPage && (
                  <RenderDetails
                    label={props.t("Refundable Amount")}
                    value={`$${jobdetails.amount_to_refund}` || "-"}
                  />
                )}
                {refunded_amount > 0 && (
                  <RenderDetails
                    label={props.t("Refunded Amount")}
                    value={`$${refunded_amount.toFixed(2)}` || "-"}
                  />
                )}
                <RenderDetails
                  label={props.t("Commissioned Amount")}
                  value={
                    `$${(
                      (jobdetails.recommended_rate *
                        jobdetails.commision_rate) /
                      100
                    ).toFixed(2)}` || "-"
                  }
                />
                <RenderDetails
                  label={props.t("Commission Rate")}
                  value={(jobdetails.commision_rate || "0") + "%"}
                />
                <RenderDetails
                ...
                <RenderDetails
                  ...
                  value={
                    `${jobdetails.street_address}, ${jobdetails.city} - ${jobdetails.zipcode}, ${jobdetails.state}` ||
                    "-"
                  }
                />
                <RenderDetails
                          ...
                          <>
                            ...
                            <div className="product-delete-text-wrapper">
                              <span className="product-delete ">
                                ...
                                {props.t(
                                  "to delete this job from job request?"
                                )}
                              </span>
                            </div>
                          </>
                            ...
                            <table className="table">
                              <tbody className="p-r tl-left">
                                <tr>
                                  ...
                                  <td>{infoDialog.address || "-"}</td>
                                </tr>
                                <tr>
                                  <td>{props.t("Citizenship")} :</td>
                                  <td>{infoDialog.citizenship || "-"}</td>
                                </tr>

                                <tr>
                                  <td>{props.t("Invited / Received")} :</td>
                                  <td>
                                    {infoDialog.is_req_send_by_requester ===
                                    1 ? (
                                      <div className="d-flex h-15">
                                        <p>Invited</p>
                                        <SentIcon />
                                      </div>
                                    ) : infoDialog.is_req_send_by_provider ===
                                      1 ? (
                                      <div className="d-flex h-15">
                                        <p>Received</p>
        ...
        <div className="customer-support-table-wrapper-new mt-40 space-between">
          <MDBDataTable
            ...
            noRecordsFoundLabel={
              <div className="no-data-found ta-l">
                {props.t("No clock-in clock-out data found!")}
              </div>
            }
            paginationLabel={[
              <MDBIcon icon="angle-left" size="sm" />,
        ...
        <div className="customer-support-table-wrapper-new mt-40 space-between">
          <MDBDataTable
            ...
            noRecordsFoundLabel={
              <div className="no-data-found ta-l">
                {props.t("No payment history found!")}
              </div>
            }
            paginationLabel={[
              <MDBIcon icon="angle-left" size="sm" />,
          ...
          <SweetAlert
            ...
            customButtons={
              <div>
                ...
                {confirmPayment.task === "pay" ? (
                  <Button className="add-button" onClick={PaymentHandler}>
                    {props.t("Pay")}
                  </Button>
                ) : (
                  <Button
                    className="add-button"
                    onClick={PaymentorRefundHandler}
                  >
                    {props.t("Refund")}
                  </Button>
                )}
          ...
          >
            <AvForm>
              ...
              >
                {confirmPayment.task === "pay" ? (
                  <>
                    {props.t("Are you sure you want to ")} {confirmPayment.task}{" "}
                    {confirmPayment.name}?{" "}
                  </>
                ) : (
                  <>
                    {props.t("You can refund to") +
                      " " +
                      (jobdetails.requester_first_name +
                        " " +
                        jobdetails.requester_last_name)}
              ...
              <div className="sweet-alert">
                <div className="form-wrapper">
                  <span className="sweet-label">
                    {props.t(
                      `${
                        confirmPayment.task == "pay" ? "Payable" : "Refundable"
                      } amount*`
                    )}
                  </span>
                  <AvField
...
export default withNamespaces()(
  connect(mapStateToProps, mapDispatchToProps)(IncompleteJobDetails)
);
